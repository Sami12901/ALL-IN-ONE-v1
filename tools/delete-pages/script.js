...
class DeletePages {
  constructor() {
    ...
    this.inDelete = document.getElementById('delete-input');
  ...
  async execute() {
    const delStr = this.inDelete.value;
    const toDelete = this.parseRange(delStr, this.totalPages);
    
    if (toDelete.length === 0) {
      alert('Please specify at least one valid page to delete.');
      return;
    }
    if (toDelete.length >= this.totalPages) {
    ...
    try {
      ...
      const copiedPages = await newPdf.copyPages(this.currentPdfDoc, pagesToKeep);
      ...
      a.href = url;
      a.download = `deleted-${this.currentFile.name}`;
      a.click();
      ...
      this.btnExecute.textContent = 'Remove Pages & Download';
    }
  }
}

document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => window.deletePages = new DeletePages(), 300);
});