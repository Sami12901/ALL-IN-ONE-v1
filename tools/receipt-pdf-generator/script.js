...
class ReceiptGenerator {
  ...
  generatePDF() {
    ...
    const fromName = document.getElementById('inv-from-name').value || 'Business Name';
    ...
    const toName = document.getElementById('inv-to-name').value || 'Customer Name';
    const toAddress = document.getElementById('inv-to-address').value;
    
    const invNum = document.getElementById('inv-num').value || 'REC-001';
    const invDate = document.getElementById('inv-date').value;
    const invMethod = document.getElementById('inv-method').value;
    ...
    doc.text(`Receipt #: ${invNum}`, 14, 30);
    doc.text(`Date Paid: ${invDate}`, 14, 35);
    doc.text(`Method: ${invMethod}`, 14, 40);
    ...
    doc.autoTable({
      ...
      body: tableBody,
      theme: 'grid',
      headStyles: { fillColor: [142, 68, 173], textColor: 255 }, // Purple theme for receipts
    ...
    doc.text('Total Paid:', 140, finalY + 24);
    doc.text(`${currency}${total.toFixed(2)}`, 196, finalY + 24, { align: 'right' });
    
    // Add Paid Stamp
    doc.setFontSize(40);
    doc.setTextColor(46, 204, 113); // Green
    // Simple way to simulate a tilted stamp in jsPDF is using standard text, but jsPDF text rotation requires matrix math.
    // Instead we just print it big.
    doc.text('PAID', 140, finalY + 45, { align: 'center' });
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => window.receiptGenerator = new ReceiptGenerator(), 300);
});