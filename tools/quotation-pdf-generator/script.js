...
class QuotationGenerator {
  ...
  setDefaultDates() {
    ...
    nextWeek.setDate(nextWeek.getDate() + 30); // Quotes usually valid for 30 days
  ...
  generatePDF() {
    ...
    const invNum = document.getElementById('inv-num').value || 'QT-001';
    ...
    doc.text(`Quotation #: ${invNum}`, 14, 30);
    doc.text(`Date: ${invDate}`, 14, 35);
    doc.text(`Valid Until: ${invDue}`, 14, 40);
    ...
    doc.autoTable({
      ...
      body: tableBody,
      theme: 'grid',
      headStyles: { fillColor: [46, 204, 113], textColor: 255 }, // Green theme for quotes
    ...
    doc.text('Estimated Total:', 140, finalY + 24);
    doc.text(`${currency}${total.toFixed(2)}`, 196, finalY + 24, { align: 'right' });

    doc.save(`Quotation_${invNum}.pdf`);
  }
}

document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => window.quotationGenerator = new QuotationGenerator(), 300);
});